...
在宣告運算子 `:=` 左邊放上欲宣告的變數名稱，右邊放上運算式，即可宣告變數。變數名稱只能使用大小寫英文字母和數字（不能用數字開頭），且同一區塊相同變數名稱只能宣告一次。運算式可以是四則運算或[函式呼叫](#函式呼叫)。
...
```
[範例](../examples/while.hzs)

如同 [If](#if) 敘述，只是加上了代表迴圈的中括號。當大括號裡的條件符合時，就會執行小括號裡面的敘述，直到條件不符合為止。
...
```
[範例](../examples/infinite-loop.hzs)

會一直重複執行小括號裡的敘述。必須搭配 [跳出敘述](#跳出敘述) 使用，否則程式不會結束。
